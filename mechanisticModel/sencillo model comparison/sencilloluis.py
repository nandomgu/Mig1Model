...
class sencillo:
    ...
    -jd for julia scripts to simulate ODEs with DifferentialEquations in the presence of an external dynamic input
    ...
    def __init__(self, fname= False):
        ...
        if fname:
            ...
            parameters= []
            variables= []
            inputs=[] # declaring inputs as special variables
            nononames= 0
                ...
                if (not line or line == 'INIT' or line == 'EQN' or line[0] == '#'
                    ...
                    if ';' in line:
                        ...
                        if bits[0] == 'parameter':
                            ...
                            # add inputs to list of inputs
                            inputs.append(' '.join(bits[1:]))
                        else:
...
#            # define y as a vector of species
    ...
    def printdriverdynamic(self, out, otype):
        '''
        Prints driver code for simulating a system of ordinary differential equations with a dynamic input.
        Arguments
        --
        out: destination for the output
        otype: 'python', 'matlab' or 'julia'
        ...
        if otype == 'julia':
            ...
            outstring= ''
            outstring2=''
            out.write('parameters= '+deliml+'\n')
            for par in p.keys():
                out.write(':'+ par + '=> ' + p[par]['value'] + fillc + '\n')
                outstring += fillc + par
            #define initial conditions as parameters
            for sname in self.alphspecies:
                if self.isfloat(s[sname]['initial amount']):
                    out.write(':'+sname + '_0=> ' + s[sname]['initial amount'] + fillc + '\n')
                else:
                    if s[sname]['initial amount'] in self.alphspecies:
                        # uses a previously defined initial amount
                        out.write(':'+sname + '_0=> ' + s[sname]['initial amount'] + '_0' + fillc + '\n')
                    else:
                        out.write(':'+ sname + '_0=> ' + s[sname]['initial amount'] + fillc + '\n')
                outstring += fillc + sname + '_0'
                outstring2 += fillc+ ':'+  sname + '_0'
            out.write(delimr+'\n\n')
            for rname in r.keys():
                # check that a rate value has been given before printing
                if r[rname]['rate value'] != '':
                    out.write(r[rname]['rate constant'] + '= ' \
                              + r[rname]['rate value'] + fillc + '\n')
                    outstring += fillc + r[rname]['rate constant']
            #out.write('parameters= ' + deliml + outstring[1:] + delimr + stp + '\n\n')
        # define initial conditions
        #out.write(commt + ' define initial conditions\n')
        #out.write('init= ' + deliml + outstring2[1:] + delimr + stp + '\n\n')
        ...
        'prob(params)= ODEProblem(model, exp.([getvalue(params, x) for x in '+ deliml+ outstring2[1:] + delimr + ']), t, exp.([j[2] for j in genotype(params)]  ))\nend\n\n')
    ...
    def printjulia(self, ofile= False):
        ...
        # function for odeint
        out.write('function ' + ofile + '(dydt, y, parameters, t)\n')
        ...
        out.write('prob= ODEProblem(' + ofile + ', init, t, parameters)\n')
    ...
    def printjuliadynamic(self, ofile= False):
        '''
        Output the model in Julia for simulation.
        A model file and a "driver" file that calls a simulation routine are created.
        ...
        #         driver code
        #         out.write('\n#######\n\n')
        driverfile=open(ofile + 'driver.jl', 'w')
        self.printdriverdynamic(driverfile, 'julia')
        #         out.write('# call solver (note args must be a tuple) \n')
        #         out.write('t= (0., 100.)\n')
        #         out.write("species= [:" + self.alphspecies[0])
        #         for sname in self.alphspecies[1:]:
