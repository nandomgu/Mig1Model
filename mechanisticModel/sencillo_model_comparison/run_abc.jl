...
@everywhere begin
    ...
    function trysolve(prob, pars, datamean, datastd) #main function to solve a system with two solvers and return a large value if both fail. 
        ...
        try
               sol = solve(prob(pars), Rosenbrock23(autodiff=false), tstops=[3.25,11.33],d_discontinuities=[3.25,11.33], saveat = interv,verbose=true,isoutofdomain=(u,p,t) -> any(x -> x < 0, u))
    ...
    function rho_lens(expd,d2)
        error = sum(trysolve.(allprobs, [d2], [j(x) for j in splines], [maximum(splines[j](tim[1:229])) .* length(x) for j in 1:18]))
...
pardistributions = [Normal(log(p[2][2]),1) for p in bounds]
pardistributions = [Uniform(log(p[2][1]), log(p[2][2])) for p in bounds]
...
for i = 1:18
    @show i
    sol = solve(allprobs[i](d2),ABDF2(), tstops=[3.25,11.33],d_discontinuities=[3.25,11.33], saveat = interv,verbose=false,isoutofdomain=(u,p,t) -> any(x -> x < 0, u),abstol=1e-12,reltol=1e-12)
    @show size(sol[1,:])
    push!(p,plot(sol[1,:],label=["model" "data"],xlabel="time",title="condition $i"))
end
...
p=[]
for i = 1:18
    sol = solve(allprobs[i](d2), ABDF2(), saveat = interv)
    push!(p,plot(sol[2,:],label=["model" "data"],xlabel="time",title="condition $i"))
end
...
vals=trysolve.(allprobs,[d2], [j(x) for j in splines], me2)
...
push!(vals, sum(trysolve.(allprobs,[d2], [j(x) for j in splines],me2) ))
...
function h(a,x,lower,upper)
    return g(a,lower+x)*g(a,upper-x)
end