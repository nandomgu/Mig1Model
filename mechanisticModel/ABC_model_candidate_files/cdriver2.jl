...
include("cgenotypes2.jl")
#Model 2. removal of mig-|mth1 interactions.

function makeproblem2(modelfile, inputx, inputy, concentration, t, h0, genotype = wt)
    ...
    function modelg2(input)
    
    
        function model2(dydt, y, parameters, t)
            ...
            # removed repression term. (1 + (Mig1/kmth1mig1)^nmth1mig1 + (Mig2/kmth1mig2)^nmth1mig2)
    ...
    prob2(params) = ODEProblem(modelg2(input), [h0;0.0;0.0;exp.(genotype(params)[[end-2, end-1]]);0.0], t, [exp(j) for j in genotype(params)])
