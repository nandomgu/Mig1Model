function handle=multiExpSimulation( modelname,outvar, initialConditions, figs,scaleFactors,normalize, varargin)
...
%eval(['[' strjoin(genvarname(repmat({'A'}, 1, numel(varargin))), ',') ']= deal(varargin{:})'])
...
for j=1:numel(outputs)
    ...
    outputs{j}= outputs{j}-min(outputs{j});
